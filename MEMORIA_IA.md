...
*   **Motor de Rescate Recursivo (v4)**: Se implementó una lógica de parseo profundo que utiliza PapaParse de forma interna para reconstruir las filas cuando el navegador falla. A diferencia de las versiones anteriores, el v4 respeta los puntos y coma (;) que están dentro de las descripciones gramaticales (entre comillas), eliminando el error de los "ceros" y garantizando que se cargue el nombre, precio y stock de forma infalible. Se añadió la marca `[Bioplast v3.1]` en la consola para confirmar el despliegue.
